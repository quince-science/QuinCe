...
#     fCO2rec_src fCO2rec_flag	-	fCO2 [uatm] QC Flag
...
# Gather list of filenames to be converted
query = input('Enter foldername or filename: * Must be located in current directory * \n')
if not os.path.exists(query): print('Can\'t locate file or folder with that name.')
else:
    if os.path.isfile(query): files+= query
    else: 
        for root,dirs,files in os.walk(query):
            if 'L2' in root:
                for file in files:
                    filenames += [root + '/' + file]
        shutil.copytree(query,now + query)
...
for filename in filenames:
    ...
    converted_filename = now + filename.replace('tsv','csv')
