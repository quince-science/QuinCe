...
public class CariocaReducer extends DataReducer {
  ...
  public void preprocess(Connection conn, Instrument instrument,
    ...
    RL = CalculationCoefficient.getCoefficient(coefficients, variable, "tempRL")
      .getValue();

    RH = CalculationCoefficient.getCoefficient(coefficients, variable, "tempRH")
      .getValue();

    R1 = CalculationCoefficient.getCoefficient(coefficients, variable, "tempR1")
      .getValue();
  ...
  public void doCalculation(Instrument instrument, Measurement measurement,
    ...
    // Get data values
    // Note that the nm values have reversed names because variables can't begin
    // with numbers
    ...
    Double R_temp = (R1 * (RH / (RH + R1)) + R1 * (K * RL) / (RL + R1)
      - (R1 * (K * RH) / (RH + R1)))
      / (1 - (RH / (RH + R1)) - ((K * RL) / (RL + R1)) + (K * RH) / (RH + R1));
    ...
    // Dissociation constants of carbonic acid in seawater (Lueker et al., 2000)
  ...
  protected String[] getRequiredTypeStrings() {
    return new String[] { "Th", "Refb", "Refh", "810nm", "596nm", "434nm" };
  }
