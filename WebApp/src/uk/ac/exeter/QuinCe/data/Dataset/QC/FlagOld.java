...
public class FlagOld {
  ...
  static {
    ...
    FLAG_SIGNIFICANCE.put(FlagOld.VALUE_FLUSHING, 70);
    FLAG_SIGNIFICANCE.put(FlagOld.VALUE_LOOKUP, 60);
    FLAG_SIGNIFICANCE.put(FlagOld.VALUE_NEEDED, 50);
    FLAG_SIGNIFICANCE.put(FlagOld.VALUE_NO_QC, 40);
    FLAG_SIGNIFICANCE.put(FlagOld.VALUE_BAD, 30);
    FLAG_SIGNIFICANCE.put(FlagOld.VALUE_QUESTIONABLE, 20);
    FLAG_SIGNIFICANCE.put(FlagOld.VALUE_NOT_CALIBRATED, 10);
    FLAG_SIGNIFICANCE.put(FlagOld.VALUE_GOOD, 00);
    FLAG_SIGNIFICANCE.put(FlagOld.VALUE_ASSUMED_GOOD, 00);
  }
   ...
   */
  public FlagOld(int flagValue) throws InvalidFlagException {
   ...
   */
  public FlagOld(char flagLetter) throws InvalidFlagException {
   ...
   *          The source flag
   */
  protected FlagOld(FlagOld sourceFlag) {
   ...
   * @return A No QC flag.
   */
  private static FlagOld makeNoQCFlag() {
    FlagOld flag = null;
    try {
      flag = new FlagOld(VALUE_NO_QC);
   ...
   * @return A Good flag.
   */
  private static FlagOld makeNotCalibratedFlag() {
    FlagOld flag = null;
    try {
      flag = new FlagOld(VALUE_NOT_CALIBRATED);
    } catch (InvalidFlagException e) {
      // This won't be thrown; do nothing
    }
   ...
   * @return A Good flag.
   */
  private static FlagOld makeGoodFlag() {
    FlagOld flag = null;
    try {
      flag = new FlagOld(VALUE_GOOD);
    } catch (InvalidFlagException e) {
      // This won't be thrown; do nothing
    }
   ...
   */
  private static FlagOld makeAssumedGoodFlag() {
    FlagOld flag = null;
    try {
      flag = new FlagOld(VALUE_ASSUMED_GOOD);
    } catch (InvalidFlagException e) {
      // This won't be thrown; do nothing
    }
   ...
   */
  private static FlagOld makeQuestionableFlag() {
    FlagOld flag = null;
    try {
      flag = new FlagOld(VALUE_QUESTIONABLE);
    } catch (InvalidFlagException e) {
      // This won't be thrown; do nothing
    }
   ...
   * @return A Bad flag.
   */
  private static FlagOld makeBadFlag() {
    FlagOld flag = null;
    try {
      flag = new FlagOld(VALUE_BAD);
    } catch (InvalidFlagException e) {
      // This won't be thrown; do nothing
    }
   ...
   */
  private static FlagOld makeNeededFlag() {
    FlagOld flag = null;
    try {
      flag = new FlagOld(VALUE_NEEDED);
    } catch (InvalidFlagException e) {
      // This won't be thrown; do nothing
    }
   ...
   */
  private static FlagOld makeFlushingFlag() {
    FlagOld flag = null;
    try {
      flag = new FlagOld(VALUE_FLUSHING);
    } catch (InvalidFlagException e) {
      // This won't be thrown; do nothing
    }
   ...
   */
  private static FlagOld makeLookupQCFlag() {
    FlagOld flag = null;
    try {
      flag = new FlagOld(VALUE_LOOKUP);
    } catch (InvalidFlagException e) {
      // This won't be thrown; do nothing
    }
  ...
  public boolean equals(Object obj) {
    ...
    if (obj == null)
      return false;
    if (!(obj instanceof FlagOld))
      return false;
    FlagOld other = (FlagOld) obj;
   ...
   */
  public boolean moreSignificantThan(FlagOld otherFlag) {
   ...
   *         flag; {@code false} if it is not.
   */
  public boolean lessSignificantThan(FlagOld otherFlag) {
    return FLAG_SIGNIFICANCE.get(flagValue) < FLAG_SIGNIFICANCE
      .get(otherFlag.flagValue);
  }
   ...
   */
  public boolean equalSignificance(FlagOld otherFlag) {
    return FLAG_SIGNIFICANCE.get(flagValue) == FLAG_SIGNIFICANCE
      .get(otherFlag.flagValue);
  }
   ...
   *
   * <p>
   * Returns the basic {@link FlagOld} object on which a subclassed
   * {@link FlagOld} is based.
   * </p>
   *
   * @return The raw Flag object.
   */
  public FlagOld getSimpleFlag() {
    return new FlagOld(this);
  }
   ...
   */
  public static FlagOld getMostSignificantFlag(FlagOld... flags) {
   ...
   * @return The most significant flag.
   */
  public static FlagOld getMostSignificantFlag(Collection<FlagOld> flags) {
    FlagOld result = null;

    for (FlagOld flag : flags) {
      if (null != flag) {
  ...
  public static boolean containsWorseFlag(Collection<FlagOld> flags,
    FlagOld reference) {
