 ...
 *
 * <p>
 * Before the job is run, the file is checked to see if it has been marked for
 * deletion. It is up to the concrete implementation of a {@code FileJob} to
...
public abstract class FileJob extends Job {
   ...
   *
   * <p>
   * The first entry in the {@code parameters} list must be the database ID of
   ...
   *           If the supplied job parameters are invalid
  ...
  protected final void execute(JobThread thread) throws JobFailedException {
    try {
      ...
      throw new JobFailedException(id,
        "Error while performing checks for file job", e);
    }
  }
